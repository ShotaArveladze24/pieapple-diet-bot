...
HELP_TEXT = (
    ...
    "/agenda - the next 7 days as a calendar view (BRE/LUN/DIN buttons per day); tap a "
    "filled slot for its recipe details plus Calendar/Remove/Replace buttons\n"
    "/recipes - alphabetical list of saved recipes with their id\n"
    "/recipe_details - names, links, per-100g nutrition, cooking info and rating for a "
    "recipe (asks for the id)\n"
    "/add_link <id> <it|es|en> - add or overwrite a recipe's link for a language\n"
    "/edit_recipe - view/edit a recipe's per-language names, links, per-100g nutrition "
    "and notes, with a Scan action to auto-fill what's missing (asks for the id)\n"
    "/scan <id> - run Scan directly on one recipe, without going through /edit_recipe\n"
    "/scan_week - run Scan on every recipe in this week's plan that's missing something\n"
    "/scan_all - run Scan on every saved recipe that's missing something\n"
    "/scan_ingredienti_IT - scrape Italian ingredients+quantities from each recipe's "
    "Italian link, for every recipe that's missing them\n"
    "/all_ingredients_IT - list every saved Italian ingredient, how many recipes use it, "
    "and the highest recipe ids among them\n"
    ...
    "/upload_photo - log a photo of a meal you're about to eat, with date/time and "
    "(optionally) a shared location, saved locally per Telegram user id\n"
    "/clean_titles - strip a stray 'VER LINK' PDF-extraction artifact from meal/recipe titles\n"
    "/dayoff - mark a day as off-plan (pick a quick date or type any other day)\n"
    "/dayon - put a day back on the plan (pick from currently marked-off days)\n"
    ...
    "/report - adherence since the start of last week, with a 1-10 rating prompt "
    "for meals you stuck to the plan for\n\n"
